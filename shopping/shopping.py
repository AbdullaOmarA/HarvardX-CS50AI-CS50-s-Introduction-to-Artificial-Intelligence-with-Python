...
def main():
    ...
    evidence, labels = lo=load_data(sys.argv[1])
...
def load_data(filename):
    ...
    with open(filename,newline='') as csvfile:
        ...
        print('Loading Data formcsv file.....')
        lines=0
        for row in readercsv:
            ...
            line.append(bool_s[row['Weekend']])


            evidence.append(line)
            labels.append(bool_s[row['Revenue']])

            if len(evidence)!= len(labels):
                sys.exit('Error when loading data!! Evidence length does not match label length')

            print('Data loading sucessfully from csv file total lines:  ',line)
...
def train_model(evidence, labels,k):
    ...
    """
    model= KNeighborsClassifier(neighbor=k)
...
def evaluate(labels, predictions):
    ...
    pos_labls=labels.count(1)
    nega_labls=labels.count(0)
    ...
    specificity= correct_pos/pos_labls
    specificity=correct_nega/nega_labls

    

    return specificity,predictions
